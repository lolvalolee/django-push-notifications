...
def _apns_create_socket_to_push(provider):
	return _apns_create_socket((SETTINGS["APNS_HOST"], SETTINGS["APNS_PORT"]), provider)


def _apns_create_socket_to_feedback(provider):
	return _apns_create_socket((SETTINGS["APNS_FEEDBACK_HOST"], SETTINGS["APNS_FEEDBACK_PORT"]), provider)
...
def _apns_send(token, alert, provider, badge=None, sound=None, category=None, content_available=False,
			   action_loc_key=None, loc_key=None, loc_args=[], extra={}, identifier=0, expiration=None, priority=10,
			   socket=None, mutable_content=False, thread_id=None):
	data = {}
	aps_data = {}
	...
	if socket:
		...
		with closing(_apns_create_socket_to_push(provider)) as socket:
			socket.write(frame)
...
def apns_send_bulk_message(registration_ids, alert, provider, **kwargs):
	...
	with closing(_apns_create_socket_to_push(provider)) as socket:
		for identifier, registration_id in enumerate(registration_ids):
			res = _apns_send(registration_id, alert, provider, identifier=identifier, socket=socket, **kwargs)
		_apns_check_errors(socket)
		return res


def apns_fetch_inactive_ids(provider):
	"""
	Queries the APNS server for id's that are no longer active since
	the last fetch
	"""
	with closing(_apns_create_socket_to_feedback(provider)) as socket:
		inactive_ids = []
